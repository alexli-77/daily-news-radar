...
[在线页面](https://alexli-77.github.io/daily-news-radar/) · [English](README.en.md) · [雷达Skill](skills/radar/README.md) · [伯乐Skill](skills/ai-news-radar/README.md) · [信息源策略](docs/SOURCE_COVERAGE.md)
...
**① 只想看AI日报** → 不用装任何东西，直接打开[在线页面](https://alexli-77.github.io/daily-news-radar/)。
...
v0.6 引入的故事线合并、AI标签分数、源健康与AI占比，仍是这一切的地基。历次改动见 [Releases](https://github.com/alexli-77/daily-news-radar/releases)。
...
仓库包含一个独立 delivery pipeline，可以把 `daily-brief.json` 生成中文人声音频。它默认使用 `edge-tts` 的中文神经声音色，不会把 mp3 提交进 Git；GitHub Actions 只保留 7 天 artifact，也可以直接推送到 Discord。
...
python scripts/build_daily_audio.py --output-dir out/daily-audio
```

默认输出：

- `out/daily-audio/daily-ai-brief.mp3`
- `out/daily-audio/daily-ai-brief-script.txt`
- `out/daily-audio/daily-ai-brief.md`

GitHub Actions 文件是 `.github/workflows/daily-audio.yml`，默认每天北京时间 08:45 生成一次。可选配置：

- Repository variable `AUDIO_TTS_VOICE`：默认 `zh-CN-YunyangNeural`
- Repository variable `AUDIO_TTS_RATE`：默认 `+0%`
- Repository variable `AUDIO_TTS_PITCH`：默认 `+0Hz`
- Repository variable `AUDIO_DATE_TIMEZONE`：默认 `Asia/Shanghai`
- Repository variable `DISCORD_CHANNEL_ID`：Discord 频道 ID
- Repository secret `DISCORD_BOT_TOKEN`：Discord Bot Token
- Repository variable `FEISHU_CHAT_ID`：飞书群 `chat_id`，形如 `oc_xxx`
- Repository secret `FEISHU_APP_ID`：飞书自建应用 App ID
- Repository secret `FEISHU_APP_SECRET`：飞书自建应用 App Secret
- Repository variable `FEISHU_API_BASE`：默认 `https://open.feishu.cn/open-apis`
- Repository secret `FEISHU_WEBHOOK_URL`：飞书群自定义机器人 webhook URL，只用于推送文字版和链接
- Repository variable `AUDIO_PUBLIC_URL`：可选，公开 mp3 地址；不配置时 webhook 会发送本次 GitHub Actions artifact 页面链接

如果不配置 Discord 或飞书，workflow 仍会生成音频 artifact。不要把 `out/` 或 mp3 文件提交进仓库，避免仓库历史再次膨胀。

飞书推送建议使用企业自建应用 Bot：开启机器人能力，把机器人拉进目标群，开通发送消息和上传文件相关权限，然后把 `FEISHU_APP_ID` / `FEISHU_APP_SECRET` / `FEISHU_CHAT_ID` 配到 GitHub。音频会以 mp3 文件附件形式发到群里，并附文字版摘要。

如果只使用飞书群自定义机器人 webhook，则可以直接推送文字版摘要和每条新闻的 URL；webhook 不能直接上传 mp3 文件。需要音频时，消息会附 GitHub Actions artifact 页面链接，或使用 `AUDIO_PUBLIC_URL` 指向外部公开 mp3。
